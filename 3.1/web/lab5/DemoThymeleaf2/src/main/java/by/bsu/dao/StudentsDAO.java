...
/*	public StudentsDAO(EntityManager entityManager) {
		...
		System.out.println("entityManager from dao =" + entityManager); 
	}
*/
	public void createDemoStudent() {
		...
		try {
			...
			student.setStartDate((java.sql.Date) Calendar.getInstance().getTime());
			// create course
	...
	public List<Student> loadStudentsByLastName(String lastName, Date d) {
		...
		public List<Student> loadStudentsInCourse (String courseName) {
			...
			for (Student st: studentList) {
				...
				for (Course stCourse: st.getCourses()) {
					System.out.println("\t\"" + 
							stCourse.getName() + "\" by " + 
							stCourse.getLector() + ", " + 
							stCourse.getStudents().size() +"students.");
				}
			}
