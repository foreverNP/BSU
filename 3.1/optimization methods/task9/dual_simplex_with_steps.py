...
def dual_simplex_method(A, b, c, bounds, j_b):
    ...
    while iteration < max_iterations:
        ...
        for j in j_h:
            ...
            delta[j] = delta_j
            print(
                f"δ_{j+1} = c_{j+1} - a_{j+1}^T * u = {c[j]} - {a_j.T} * {u} = {delta_j}"
            )
        ...
        try:
            ...
            print(
                "Матрица A_B вырожденная, невозможно решить для базисных переменных x_B."
            )
            return None
        x_e[j_b] = x_e_B
    ...
    print(
        "Достигнуто максимальное количество итераций без нахождения оптимального решения."
    )
    return None
